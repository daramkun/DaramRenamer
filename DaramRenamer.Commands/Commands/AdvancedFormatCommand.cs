...
internal class AdvancedFormatCommand : ICommand, IOrderBy
{
    ...
    private abstract class FormatStringNode
    {
        ...
        public FormatStringNode(string token)
        {
            Token = token;
        }
    ...
    private class EnvironmentVariableNode : FormatStringNode
    {
        ...
        {
            return token.StartsWith("env:");
        }

        public override string GetValue(FileInfo fileInfo)
        {
            return Environment.GetEnvironmentVariable(VariableName, VariableTarget);
        }
    }
